...
class UserManager(models.Manager):
	...
	def register(self, first_name, last_name, dateofbirth, email, password, confirm_password ):
		errors = []
		if (len(first_name) == 0) or (len(last_name) == 0) or (len(dateofbirth) == 0)  or (len(email) == 0) or (len(password) == 0):
		...
		if len(errors) is not 0:
			...
			new_user = User.objects.create(first_name=first_name, last_name=last_name, dateofbirth=dateofbirth, email=email, password=pw_hash)
